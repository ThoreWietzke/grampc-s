...
namespace grampc_s
{
    ...
    : inputDim_(data.inputDimension),
      ...
      numStates_(stateDependency.size()),
      numControls_(controlDependency.size()),
      numStateArguments_(std::count(stateDependency.begin(), stateDependency.end(), true)),
      numControlArguments_(std::count(controlDependency.begin(), controlDependency.end(), true)),
